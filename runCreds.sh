...
osascript -e "tell application \"Terminal\"
    do script \"cd '$BACKEND_DIR' && echo -e '\\033[0;32m🔧 Starting Crosslake Backend...\\033[0m' && echo && if [ -f .env ]; then source venv/bin/activate 2>/dev/null || python3 -m venv venv && source venv/bin/activate && pip install -r requirements.txt && python main.py; else echo -e '\\033[0;31m❌ No .env file found! Please create one from .env.example\\033[0m'; fi\"
...
osascript -e "tell application \"Terminal\"
    do script \"cd '$FRONTEND_DIR' && echo -e '\\033[0;34m🎨 Starting Crosslake Frontend...\\033[0m' && echo && if [ -f .env.local ]; then npm install && npm run dev; else echo -e '\\033[0;31m❌ No .env.local file found! Please create one from .env.example\\033[0m'; fi\"
end tell" >/dev/null 2>&1
